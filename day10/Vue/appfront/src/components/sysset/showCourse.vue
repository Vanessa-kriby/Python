<template>
  <span>
    ...
    <el-table :data="listData" style="width:100%">
      ...
      <el-table-column prop="category" label="学生性别" width="180">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    checkcorrect(item) {
      ...
      if (!(tbook.length > 0 && tbook.length < 20)) {
        return false;
      }
      let tcategory = item.category.trim();
      if (!(tcategory.length > 0 && tcategory.length < 20)) {
        return false;
      }
      return true;
    },
      ...
      this.listData.map((item) => {
        if (item.status) {
          if (!this.checkcorrect(item))
            return this.$message({
              message: "添加失败！" + item.cno + item.cname + item.textBook + item.category,
              type: "waring",
            });
          item.status = 0;
          var vm = new Object();
    ...
    onSubmit(data) {
      ...
      // console.log(data);
      this.axios
        .post("http://127.0.0.1:8000/app2/addCourses", { data })
        .then((response) => {
    ...
    getCourse: function (event) {
      this.axios
        .get("http://127.0.0.1:8000/app2/showCourses")
        .then((response) => {
          this.listData = response.data.data; 
    ...
    handleUpdate(index, row) {
      ...
      if (!this.checkcorrect(row))
        return this.$message({
          message: "失败" + row.cno + row.cname+row.textBook,
          type: "warning",
        });
      let vm = new Object();
      vm.cno = row.cno;
    ...
    handleDelete(index, row) {
      ...
      this.axios
        .post("http://127.0.0.1:8000/app2/delCourse", { data })
        .then((response) => {
          if (response.data.err_num == 1) {
            this.$message({
