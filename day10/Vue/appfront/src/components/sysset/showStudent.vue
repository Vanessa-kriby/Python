  ...
  <span>
    ...
    <el-table :data="listData" style="width:100%">
        ...
        <el-table-column prop="nation" label="民族" width="180">
            <template slot-scope="scope">
                <el-input v-if="scope.row.status" v-model="scope.row.nation"></el-input>
                <span v-else>{{scope.row.nation}}</span>
            </template>
        </el-table-column>

        <el-table-column prop="native_place" label="籍贯" width="180">
            <template slot-scope="scope">
                <el-input v-if="scope.row.status" v-model="scope.row.native_place"></el-input>
                <span v-else>{{scope.row.native_place}}</span>
            </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    checkcorrect(item) {
      ...
      if (!(sgen.length > 0 && sgen.length < 20)) {
        return false;
      }
      ...
      let nation = item.nation.trim();
      if (!(nation.length > 0 && nation.length < 20)) {
        return false;
      }
      if (item.native_place == " ") {
        return false;
      }
      let native_place = item.native_place.trim();
      if (!(native_place.length > 0 && native_place.length < 20)) {
        return false;
      }
      return true;
    },
    ...
    getDepartment:function(event){
                this.axios.get('http://127.0.0.1:8000/app1/showDepartments')
                .then((response) => {
